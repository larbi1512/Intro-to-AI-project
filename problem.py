...
class Problem:
    ...
    def depth_limited_search(self, start_node, goal_nodes, depth_limit):
        # Search the deepest nodes in the search tree first using depth-limited search.
        # Returns the path to any of the goal nodes if found within the depth limit, otherwise returns None.

        def recursive_dls(node, depth):
            if depth == 0 and node in goal_nodes:
                return [node]
            elif depth > 0:
    ...
    def iterative_deepening_depth_first_search(self, start_node, goal_nodes, max_depth):
        for depth in range(1, max_depth + 1):
            for goal_node in goal_nodes:
                result = depth_limited_search(
                    problem, start_node, goal_node, depth)
                if result is not None:
                    return result
        return None
    ...
    def bidirectional_search(self, start, goal):
        ...
        backward_graphs = {goal_node: self.graph.subgraph([goal_node]) for goal_node in goal_nodes}


    # Initialize the sets of explored nodes for each direction
        forward_explored = set([start])
        backward_explored = {goal_node: set([goal_node])for goal_node in goal_nodes}

    # Initialize the queue of nodes to explore for each direction
        forward_queue = [start]
        backward_queues = {goal_node: [goal_node] for goal_node in goal_nodes}

    # Loop until the two search frontiers meet
        while forward_queue and any(backward_queue.values()):
        # Check if there is an intersection of the forward and backward explored sets
            intersection = forward_explored.intersection(set.union(*backward_explored.values()))
            if intersection:
            # We have found a path from start to goal
                goal_node = next(iter(intersection))
                path = []
                node = goal_node
            # Follow the path from the goal node to the start node
                while node != start_node:
                    path.append(node)
                    node = next(n for n in forward_graph.predecessors(node))
                path.append(start_node)
                path.reverse()
            # Follow the path from the goal node to the corresponding end node
                node = next(n for n in backward_graphs[goal_node].predecessors(goal_node))
                while node != goal_node:
                    path.append(node)
                    node = next(n for n in backward_graphs[goal_node].predecessors(node))
