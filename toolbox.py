...
class toolbox:
    ...
    def animate_solution(self, path):
        ...
        def update(frame):
            ...
            node_colors = ['green' if node in nodes_to_color else 'gray' for node in self.__problem.graph.nodes()]

            edge_labels = nx.get_edge_attributes(self.__problem.graph, 'weight')
            print(edge_labels)
        ...
        ani = FuncAnimation(plt.gcf(), update, frames=len(list(path)), interval=1000)
